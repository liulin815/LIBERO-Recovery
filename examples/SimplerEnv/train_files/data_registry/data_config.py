...
class OxeBridgeDataConfig:
    ...
    observation_indices = list(range(17))
    action_indices = list(range(16))
...
class OxeRT1DataConfig:
    ...
    # NOTE: must match the dataset meta/modality.json of
    # fractal20220817_data_*_lerobot, whose state uses euler angles + pad
    # (x, y, z, roll, pitch, yaw, pad, gripper) — NOT quaternions.
    state_keys = [
        "state.x", "state.y", "state.z",
        "state.roll", "state.pitch", "state.yaw",
        "state.pad", "state.gripper",
