...
SAVE_EVAL_DATA=0
SAVE_MODE="all"
EVAL_DATA_SAVE_DIR="/H20_vepfs/liulin/starVLA/datasets/eval_output_v2"
...
ALL_TASK_TYPES=("demo_clean" "demo_randomized")
...
run_one_eval() {
  ...
  while [[ ${attempt} -le ${RETRY_TIMES} ]]; do
    ...
    local before_count
    before_count=$(find "${ROBOTWIN_PATH}/eval_result/${task}/model2robotwin_interface/${task_type}/${algo}" -name "_result.txt" 2>/dev/null | wc -l || true)
    ...
    local latest_file
    latest_file=$(find "${ROBOTWIN_PATH}/eval_result/${task}/model2robotwin_interface/${task_type}/${algo}" -name "_result.txt" 2>/dev/null | sort | tail -n 1 || true)
    ...
    local after_count
    after_count=$(find "${ROBOTWIN_PATH}/eval_result/${task}/model2robotwin_interface/${task_type}/${algo}" -name "_result.txt" 2>/dev/null | wc -l || true)
