...
def collect_human_trajectory(
    env, device, arm, env_configuration, problem_info, remove_directory=[], init_sim_state=None
):
    """
    ...
    Args:
        ...
        init_sim_state (np.ndarray | None): if provided, restore full MuJoCo sim state after reset
    """
    ...
    # Restore full sim state (objects + robot) from a saved snapshot
    if init_sim_state is not None:
        env.env.sim.set_state_from_flattened(init_sim_state)
        env.env.sim.forward()
        # Sync controller to the restored robot pose so actions are relative to current state
        robot = env.env.robots[0]
        robot.controller.update(force=True)
        robot.controller.reset_goal()
        print("[INFO] Restored sim state from file. Robot + objects at saved positions.")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--init-state-file", type=str, default=None,
                        help="Path to a .npy file containing full MuJoCo sim state to restore after reset")
    ...
    # Load init sim state if provided
    init_sim_state = None
    if args.init_state_file is not None:
        assert os.path.exists(args.init_state_file), f"State file not found: {args.init_state_file}"
        init_sim_state = np.load(args.init_state_file)
        print(f"[INFO] Loaded sim state from {args.init_state_file} (shape={init_sim_state.shape})")
